...
router.post(
  ...
  [
    ...
    check("nombreUsuario", "El nombre solo puede contener letras y espacios").matches(/^[a-zA-ZáéíóúÁÉÍÓÚñÑ\s]+$/),
    check("correo", "El correo no es valido").isEmail(),
    check("contraseña", "La contraseña debe tener al menos 6 caracteres").isLength({ min: 6 }),
    check("telefono", "El teléfono es obligatorio").not().isEmpty(),
    check("telefono", "El teléfono debe tener entre 7 y 15 dígitos").isLength({ min: 7, max: 15 }).isNumeric(),
    check("DNI", "El DNI es obligatorio").not().isEmpty(),
    check("DNI", "El DNI debe tener 8 dígitos").isLength({ min: 8, max: 8 }).isNumeric(),
